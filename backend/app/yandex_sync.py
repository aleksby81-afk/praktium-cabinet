...
def sync_all():
    ...
    try:
        with lock, SessionLocal() as db, httpx.Client(timeout=30, follow_redirects=True) as client:
            ...
            folder_path = settings.yandex_disk_folder.rstrip('/')
            folder = client.put(
                API,
            ...
            upload_snapshot(
                ...
                [[o.id, o.client_id, o.client.email, o.product_id, o.product.title, o.quantity, str(o.unit_price), str(o.unit_price * o.quantity), o.status, o.comment, o.created_at.isoformat()] for o in orders],
            )
            upload_snapshot(
                client,
                'bonuses.xlsx',
                ['event_id', 'client_id', 'client_name', 'client_email', 'amount', 'reason', 'created_at'],
                [[b.id, b.client_id, b.client.name, b.client.email, str(b.amount), b.reason, b.created_at.isoformat()] for b in bonuses],
            )
